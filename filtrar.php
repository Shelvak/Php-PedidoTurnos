...
$query_contacto1 = sprintf ("SELECT * FROM tb_contactos LEFT JOIN tb_grupos ON tb_contactos.idGrupo = tb_grupos.idGrupo WHERE Apellido LIKE '%%%s%%'",$busca);
$query_limit_contacto1 = sprintf("%s LIMIT %d, %d", $query_contacto1, $startRow_contacto1, $maxRows_contacto1);
$contacto1 = mysql_query($query_contacto1, $coso1) or die(mysql_error());
$row_contacto1 = mysql_fetch_assoc($contacto1);
$condi = mysql_num_rows($contacto1);
}
if ($_POST['filtro'] == 'nombre') {
$query_contacto1 = sprintf ("SELECT * FROM tb_contactos LEFT JOIN tb_grupos ON tb_contactos.idGrupo = tb_grupos.idGrupo WHERE Nombre LIKE '%%%s%%'",$busca);
$query_limit_contacto1 = sprintf("%s LIMIT %d, %d", $query_contacto1, $startRow_contacto1, $maxRows_contacto1);
...
?>
        ...
        <table width="95%" border="1" align="center">
          <tr>
            ...
            <th colspan="2" bgcolor="#555555" scope="col">Modificar</th>            
          </tr>
          <?php do { 
		  ?>
          <tr> 
            ...
            <td><?php echo $row_contacto1 ['Appdo']; ?></td>
            <td><?php echo $row_contacto1 ['telefono']; ?></td>
            <td><?php echo $row_contacto1 ['celular']; ?></td>
            <td><?php if ($row_contacto1 ['idGrupo'] != 0)
			echo $row_contacto1 ['nombreGrupo'];
			else echo " --- ";?></td></td>
            <td><a href="mod-cont.php?id=<?php echo $row_contacto1 ['idContacto'] ?>"><img src="images/editar.png" width="16" height="16"/></a></td>
            <td><a href="elim-cont.php?id=<?php echo $row_contacto1 ['idContacto'] ?>"><img src="images/borrar.png" width="16" height="16"/></a></td>
          </tr>
          <?php } while ($row_contacto1 = mysql_fetch_assoc($contacto1)); ?>
        </table>
      </form>
      <?php }
	  else echo "Filtro vacio, ingresa otro por favor"; ?>
